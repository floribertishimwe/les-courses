        ...
        <div className="flex gap-8 xl:gap-10">
          ...
          <main className="flex-1">
            ...
            <div className=" flex items-center justify-between mb-6 " >
              ...
              <button  onClick={()=> setMobileFiltersOpen(true)} className=" lg:hidden flex items-center gap-2 px-3 py-2
              text-sm bg-white rounded-xl border border-app-border 
              ...
              <div className="relative">
                <select value={sort}  onChange={(e)=>updateFilter("sort",e.target.value)} className=" appearance-none pl-3 pr-8 py-2 
                text-sm bg-white rounded-xl border border-app-border focus: border-app-green outline-none cursor-pointer">
                ...
                <ChevronDownIcon  className="absolute right-2.5 top-1/2 -translate-y-1/2
            ...
            { loading? ( 
              ...
              <div  className="text-center py-16">
                ...
                <button  onClick={clearFilters}  className=" px-5 py-2 text-sm font-medium bg-app-green
                text-white rounded-xl hover: bg-app-green-light transition-colors"> Clear Filters </button>
            ...
            { totalPages > 1 && ( 
              ...
              { Array.from({ length: totalPages }).map(( _ , index)=>(
                ...
                <button  key={ index } onClick={ ()=> { updateFilter("page", String( index + 1)); scrollTo(0,0) } }
                className={`size-9 rounded-lg text-sm  font-medium transition-colors ${page === index + 1 ? " bg-app-green text-white" : " bg-white text-app-text-light hover:bg-app-cream"}`}>
                { index + 1  }
                </button>
              ))}
