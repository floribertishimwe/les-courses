  ...
  return (
    <div className="min-h-screen mb-20 bg-app-cream">
      <div className="max-w-6xl mx-auto px-4  sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <button onClick={()=>navigate("/orders") }  className=" flex items-center gap-2 text-sm text-app-text-light 
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
          <h1 className="text-2xl font-semibold text-app-green ">Order#{ order!._id.slice(-8).toLocaleUpperCase()}</h1>
          ...
          <span className={` px-4 py-1.5 text-sm font-semibold rounded-full ${ order!.status === 'Delivered'?"bg-green-100 text-green-700 ": order!.status === "Canceled" ?" bg-red-100 text-red-700": "bg-app-orange/10 text-app-orange " }`} >
            { order!.status }
          </span>
        </div>
        ...
        <div className=" grid lg:grid-cols-3  gap-6  " >
            ...
            { order?.deliveryPartner && order.status !== "Delivered" && order.status !== "Cancelled" &&(
              <div className=" bg-white rounded-2xl p-5 flex items-center justify-between">
                ...
                <a href={ `tel: ${order.deliveryPartner.phone}`} className="p-2.5 bg-app-cream rounded-xl hover:bg-app-cream-dark transition-colors  "> 
                  <PhoneIcon className=" size-4 text-app-green "/> </a>
          ...
          <div className="space-y-5">
            ...
            <div className=" bg-white rounded-2xl p-5">
              ...
              <p className="text-sm text-app-text-light loading-relaxed">
            ...
            <div className="mt-4 pt-3 border-t borber-app-border space-y-1.5 text-sm">
