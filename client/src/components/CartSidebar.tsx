          ...
          { items.length === 0 ? (
              ...
              <div key={ item.product._id }  className="flex gap-3 bg-app-cream/60 rounded-xl p-3">
                ...
                <div  className=" flex-1 min-w-0 " >
                  ...
                  <p  className=" text-xs " > { currency } { item.product.price.toFixed(2) } / { item.product.unit }</p>
          ...
          { items.length > 0 && (
             <div className="p-5 border-t border-app-border space-y-3">
                ...
                { deliveryFee > 0 && <p className=" text-xs text-app-text-light text-center" 
                 ...
                 <button  onClick={()=>{ setIsCartOpen(false); navigate('/checkout'); window.scrollTo(0,0)}} className=" w-full py-3 bg-app-orange text-white font-semibold rounded-xl 
