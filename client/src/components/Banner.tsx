...
const Banner = () => {
    ...
    const [bannerVisible, setBannerVisible] = useState(()=> {  
        return sessionStorage.getItem('banner_disimissed' ) !== "true"
        });  
        const dismissBanner = () =>{
          setBannerVisible(false);
          sessionStorage.setItem('banner_disimissed','true');
        }
  return (
    <div>
      <div>
          ...
          <div className="bg-linear-to-r from-app-green via-emerald-800 to-app-green text-white text-xs sm:text-sm relative overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex-center gap-6">
              <div className="flex-center gap-2">
                ...
                <span className="font-medium">Free delivery on the orders above 20k rwf</span>
              </div>
               <span className="hidden sm:inline text-white/40">|</span>
               <div className="hidden sm:flex items-center gap-2 " > 
                  <ZapIcon  className=" size-3.5  fill-yellow-400 text-yellow-400 shrink-0"/>
                  <span className="text-xs font-semibold">Farm fresh products delivered daily</span>
               </div>
            </div>
