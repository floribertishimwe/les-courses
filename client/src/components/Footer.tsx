        ...
        <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div>
                  ...
                  <div className="flex gap-3 ">
                    { footerData.brand.socials.map((socials, i)=>(
                    <a key={i} href={ socials.link } className=" size-9 rounded-lg bg-white/10 flex-center hover:bg-white/2"> <socials.icon className="size-4"/></a>
                    )) }
                ...
                <div key={i}>
                    ...
                    <ul className="space-y-2.5 " >
                        { section.links.map( (link, i)=>(
                            <li key={i}>
                               {link.to? ( 
                                <Link to={ link.to}  className=" text-sm text-white/70 hover:text-white " >{link.label}
                                </Link>
